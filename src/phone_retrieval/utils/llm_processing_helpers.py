...
def extract_json_from_text(text_output: Optional[str]) -> Optional[str]:
    ...
    def _extract_balanced_json(candidate_text: str) -> Optional[str]:
        ...
        for start_idx in start_positions:
            ...
            # Try the next possible JSON start if this one never balanced.
        return None
...
def adapt_schema_for_openai(pydantic_model_cls: Type[BaseModel]) -> Dict[str, Any]:
    """
    Adapt a Pydantic schema for OpenAI Responses API JSON-schema mode.

    Goals:
    - strip noisy metadata like `title` and `default`
    - collapse Optional[X] anyOf[X, null] shapes where possible
    - set `additionalProperties=False` on object nodes for stricter output
    """
    model_schema = pydantic_model_cls.model_json_schema()
    ...
    def _transform(node: Any) -> Any:
        ...
        if isinstance(any_of, list):
            ...
            if len(non_null) == 1 and isinstance(non_null[0], dict):
                replacement = dict(non_null[0])
                for preserved_key in ("description",):
                    if preserved_key in cleaned and preserved_key not in replacement:
                        replacement[preserved_key] = cleaned[preserved_key]
    ...
    adapted = _transform(model_schema)
    logger.debug(f"Adapted OpenAI schema for {pydantic_model_cls.__name__}: {json.dumps(adapted, indent=2)}")
    return adapted